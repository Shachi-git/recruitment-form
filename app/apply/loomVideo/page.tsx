...
import DeliverySection from '@/components/sections/loomVideo/delivery-section'
import { Button } from '@/components/ui/button'
import { openPositions } from '@/lib/applicationPosition'
import React, { useEffect, useRef, useState } from 'react'
import { loomVideoFormSchema } from '@/lib/formSchema'
...
const Page = () => {
  ...
  const LOCAL_STORAGE_KEY = 'loomVideoFormData'
  ...
  const handleNext = () => {
    const requiredFields =
      step === 1
        ? ['email', 'firstName', 'lastName', 'whatsapp', 'position']
        : []
  ...
  const handleSubmit = async () => {
    try {
      const result = loomVideoFormSchema.safeParse(formData)
      ...
      const res = await fetch('/api/loomVideo', {
        method: 'POST',
        headers: {
      ...
      toast({
        ...
        description:
          'Your Loom Video form has been submitted successfully. Thank you for your time and interest.',
        variant: 'success',
      })
    } catch (error) {
    ...
    <div className="w-full min-h-full flex flex-col items-center justify-center">
      {step === 1 && (
        <div className="w-full flex flex-col gap-5">
          <InfoContainer
            id="email"
            question="Your Email Address used for this campaign:"
            required
            type="email"
